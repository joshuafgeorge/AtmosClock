...
public class ClockGui extends JFrame implements MouseListener{
	...
	private void drawnum(Graphics g, float angle,int n) {
		...
		if(Integer.parseInt(dtfspec.format(now)) == 000002){
			AtmosClockPre.atmos(str);
		...
		if (Integer.parseInt(dtf.format(now)) == 0 && n == 12) {
			n1 -= 12;
			System.out.print("midnight correction");
		}
		...
		else if (AtmosClockPre.getColor(n + n1).equalsIgnoreCase("dark blue")) {
			g.setColor(Color.BLUE);
		}
		else if (AtmosClockPre.getColor(n + n1).equalsIgnoreCase("cyan")) {
			g.setColor(Color.CYAN);
		}
		else if (AtmosClockPre.getColor(n + n1).equalsIgnoreCase("yellow")) {
			g.setColor(Color.YELLOW);
		}
		else if (AtmosClockPre.getColor(n + n1).equalsIgnoreCase("orange")) {
			g.setColor(Color.ORANGE);
		}
		else if (AtmosClockPre.getColor(n + n1).equalsIgnoreCase("red")) {
			g.setColor(Color.RED);
		}

		g.fillOval(centerX-10 +(int)((size/2-20-50) * -sine), centerY-10 + (int)((size/2-20-50) * -cosine), 20, 20);

		if (AtmosClockPre.getRaining(n + n1)) {
			g.setColor(new Color(4,163,181));
		} else if (!AtmosClockPre.getRaining(n + n1)) {
